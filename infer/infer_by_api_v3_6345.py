...
# deepseek_output_path = os.path.join(base_dir, "output/deepseek/result2.csv")
input_path = os.path.join(base_dir, "data/text_data/dev.csv")  # 使用ASR结果作为输入
deepseek_output_path = os.path.join(base_dir, "output/deepseek/dev_p3_t13.csv")
...
class DeepSeekTranslator:
    ...
    def create_system_prompt(self, source_lang, target_lang):
        """创建系统提示词"""
        system_prompt = f"""你是一个专业的{source_lang}译{target_lang}的翻译专家，擅长将{source_lang}翻译成{target_lang}。用户将输入{source_lang}文本，请只返回翻译后的{target_lang}文本，要求翻译后的{target_lang}文本与原文{source_lang}文本语义高度一致，且尽量是{target_lang}口语化的表述，不要包含除了翻译结果以外的任何解释或额外信息。"""
