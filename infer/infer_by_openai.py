...
def get_output_path(n_shot):
    if dev_mode:
        return os.path.join(base_dir, f"output/openai/dev_4o_{n_shot}shot.csv")
    else:
        return os.path.join(base_dir, f"output/openai/result.csv")
...
# GPT-4o-mini API配置
API_KEYS = ["sk-vjTxlt8Em7DMe00UIt168ii1B5LxVRH7KCq5zrNREYS7YlKMfrOzP5T0KzCN-I-4op4t5a4JSvkCUHOvW9bmBQbz4RF0"]  # 单个OpenAI API key
BASE_URL = "https://openapi.monica.im/v1/"
MODEL_NAME = "gpt-4o"

# 并发配置
MAX_CONCURRENT_REQUESTS = 5  # 默认并发数，可通过参数设置
...
class OpenAIKeyManager:
    def __init__(self, api_keys: List[str]):
        ...
        self._lock = Lock()
        print(f"初始化OpenAI Key Manager，共 {len(api_keys)} 个API keys")
...
def load_bleu2_cache():
    ...
    if not os.path.exists(bleu2_cache_file):
        ...
        print("请先运行预计算: python infer_by_openai.py --precompute-bleu2")
        return None
...
def translate_gpt(n_shot=0):
    """主翻译函数"""
    # 设置信号处理器
    signal.signal(signal.SIGINT, signal_handler)
    
    print("开始使用OpenAI GPT-4o-mini API进行翻译...")
    print("提示：如果程序卡住，可以按 Ctrl+C 强制终止")
    if n_shot > 0:
        print(f"使用 {n_shot}-shot 学习模式")
    
    # 获取输出路径
    gpt_output_path = get_output_path(n_shot)
    
    # 确保输出目录存在
    os.makedirs(os.path.dirname(gpt_output_path), exist_ok=True)
    
    # 检查API密钥
    if API_KEYS[0] == "your-openai-api-key-here":
        print("警告：请在脚本中设置您的OpenAI API密钥！")
    ...
    if os.path.exists(gpt_output_path):
        print(f"发现已存在结果文件: {gpt_output_path}")
        existing_df = pd.read_csv(gpt_output_path)
    ...
    # 创建OpenAI翻译器
    translator = OpenAITranslator(API_KEYS, BASE_URL, MODEL_NAME, n_shot=n_shot)
    ...
    with tqdm(total=len(texts_and_langs_with_idx), desc="总翻译进度") as pbar:
        for batch_idx in range(total_batches):
            ...
            # 保存当前进度
            df.to_csv(gpt_output_path, index=False)
    ...
    print(f"最终结果已保存到: {gpt_output_path}")
...
def precompute_similarity_ranking():
    ...
    if bleu2_cache is None:
        print("请先运行BLEU2预计算: python infer_by_openai.py --precompute-bleu2")
        return
...
def diagnose_api_connection():
    ...
    print(f"   第一个API_KEY前缀: {API_KEYS[0][:10]}..." if API_KEYS[0] != "your-openai-api-key-here" else "   ⚠️ 请设置真实的API密钥!")
    ...
    try:
        import requests
        # 测试对OpenAI API的连接
        test_url = "https://openapi.monica.im"
        response = requests.get(test_url, timeout=10)
        print(f"   连接到 {test_url}: ✓ 成功 (状态码: {response.status_code})")
    except Exception as e:
        print(f"   连接到OpenAI API: ✗ 失败 - {e}")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="使用OpenAI API进行翻译")
    parser.add_argument("--test", action="store_true", help="运行翻译测试")
    parser.add_argument("--api-keys", nargs='+', help="OpenAI API密钥列表")
