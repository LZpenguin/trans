...
def get_output_path(n_shot):
    if dev_mode:
        return os.path.join(base_dir, f"output/gemini/dev_gp_en_{n_shot}shot.csv" if n_shot > 0 else "output/gemini/dev_g.csv")
    else:
...
class GeminiTranslator:
    def __init__(self, api_keys, base_url, model_name, n_shot=0, use_english_prompt=False):
        self.api_keys = api_keys
    ...
    def create_system_prompt(self, source_lang, target_lang, current_text=None, dev_idx=None, use_english_prompt=False):
        ...
        if use_english_prompt:
            # 使用英语提示词
            source_lang_en = lang_to_english.get(source_lang, source_lang)
            target_lang_en = lang_to_english.get(target_lang, target_lang)
            
            if self.n_shot == 0:
                # 英语系统提示词（无示例）
                system_prompt = f"""You are a professional multilingual translation expert, specialized in translating from {source_lang_en} to {target_lang_en}. The user will input Chinese text, and you should only return the translated {target_lang_en} text without any explanations or additional information."""
            else:
                # 包含示例的英语系统提示词
                examples = self.load_examples(target_lang, current_text, dev_idx)
                
                if examples:
                    examples_text = "\n\nHere are some translation examples for reference (sorted by similarity):\n"
                    for i, (chinese, translated) in enumerate(examples, 1):
                        examples_text += f"Example {i}:\n"
                        examples_text += f"Chinese: {chinese}\n"
                        examples_text += f"{target_lang_en}: {translated}\n\n"
                    
                    system_prompt = f"""You are a professional multilingual translation expert, specialized in translating from {source_lang_en} to {target_lang_en}.{examples_text}Please refer to the above examples for translation style and quality. Translate the user's Chinese text input to {target_lang_en}. Only return the translated {target_lang_en} text without any explanations or additional information."""
                else:
                    # 如果没有找到示例，使用原有英语提示词
                    system_prompt = f"""You are a professional multilingual translation expert, specialized in translating from {source_lang_en} to {target_lang_en}. The user will input Chinese text, and you should only return the translated {target_lang_en} text without any explanations or additional information."""
        else:
    ...
    def translate_text(self, text, source_lang, target_lang, dev_idx=None):
        ...
        for attempt in range(MAX_RETRIES + 1):
            try:
                ...
                system_prompt = self.create_system_prompt(source_lang, target_lang, text, dev_idx, self.use_english_prompt)
...
def translate_gemini(n_shot=0, use_english_prompt=False):
    """主翻译函数"""
    # 设置信号处理器
    ...
    # 创建Gemini翻译器
    translator = GeminiTranslator(API_KEYS, BASE_URL, MODEL_NAME, n_shot=n_shot, use_english_prompt=use_english_prompt)
...
def test_translation(n_shot=0, use_english_prompt=False):
    """测试翻译功能"""
    print("测试Gemini翻译...")
    if n_shot > 0:
    ...
    for i, (text, source_lang, target_lang) in enumerate(test_cases, 1):
        ...
        # 获取完整的系统提示词
        system_prompt = translator.create_system_prompt(source_lang, target_lang, text, use_english_prompt=use_english_prompt)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--diagnose", action="store_true",
                       help="诊断API连接和配置问题")
    parser.add_argument("--english-prompt", action="store_true", default=True,
                       help="使用英语提示词（可能提升翻译质量）")
    ...
    if args.precompute_all:
        ...
        test_translation(n_shot=n_shot, use_english_prompt=args.english_prompt)
    else:
        translate_gemini(n_shot=n_shot, use_english_prompt=args.english_prompt) 