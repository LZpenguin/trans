...
def get_output_path(n_shot):
    if dev_mode:
        return os.path.join(base_dir, f"output/deepseek/dev_p2_t0_rag_ml_{n_shot}shot.csv")
    else:
...
class DeepSeekTranslator:
    def __init__(self, api_key, base_url, model_name, n_shot=0, use_multilingual_prompts=False):
        self.client = OpenAI(
            api_key=api_key,
            base_url=base_url
        )
        ...
        self.n_shot = n_shot
        self.use_multilingual_prompts = use_multilingual_prompts  # 是否使用多语言提示词
        ...
        # 打印配置信息
        if self.use_multilingual_prompts:
            print(f"✓ 启用多语言提示词模式")
        else:
            print(f"✓ 使用中文提示词模式")
    ...
    def get_language_specific_prompts(self, target_lang):
        """根据目标语言返回相应的提示词模板"""
        prompts = {
            "英语": {
                "system_base": "You are a professional multilingual translation expert, skilled at translating Chinese to English. Please only return the translated English text, without any explanations or additional information.",
                "system_with_examples": "You are a professional multilingual translation expert, skilled at translating Chinese to English. {examples_text}Please refer to the above examples' translation style and quality, and translate the user's Chinese text into English. Please only return the translated English text, without any explanations or additional information.",
                "example_label": "Example",
                "chinese_label": "Chinese",
                "target_label": "English"
            },
            "马来语": {
                "system_base": "Anda adalah pakar terjemahan multibahasa profesional, mahir dalam menterjemahkan bahasa Cina ke bahasa Melayu. Sila hanya kembalikan teks bahasa Melayu yang diterjemahkan, tanpa sebarang penjelasan atau maklumat tambahan.",
                "system_with_examples": "Anda adalah pakar terjemahan multibahasa profesional, mahir dalam menterjemahkan bahasa Cina ke bahasa Melayu. {examples_text}Sila rujuk gaya dan kualiti terjemahan contoh di atas, dan terjemahkan teks bahasa Cina pengguna ke bahasa Melayu. Sila hanya kembalikan teks bahasa Melayu yang diterjemahkan, tanpa sebarang penjelasan atau maklumat tambahan.",
                "example_label": "Contoh",
                "chinese_label": "Cina",
                "target_label": "Melayu"
            },
            "泰语": {
                "system_base": "คุณเป็นผู้เชี่ยวชาญการแปลภาษาหลายภาษาระดับมืออาชีพ ที่มีความชำนาญในการแปลจากภาษาจีนเป็นภาษาไทย กรุณาส่งคืนเฉพาะข้อความภาษาไทยที่แปลแล้วเท่านั้น โดยไม่ต้องมีคำอธิบายหรือข้อมูลเพิ่มเติมใดๆ",
                "system_with_examples": "คุณเป็นผู้เชี่ยวชาญการแปลภาษาหลายภาษาระดับมืออาชีพ ที่มีความชำนาญในการแปลจากภาษาจีนเป็นภาษาไทย {examples_text}กรุณาอ้างอิงรูปแบบและคุณภาพการแปลจากตัวอย่างข้างต้น และแปลข้อความภาษาจีนของผู้ใช้เป็นภาษาไทย กรุณาส่งคืนเฉพาะข้อความภาษาไทยที่แปลแล้วเท่านั้น โดยไม่ต้องมีคำอธิบายหรือข้อมูลเพิ่มเติมใดๆ",
        ...
        # 如果没有找到对应语言，使用中文作为默认
        return prompts.get(target_lang, {
            "system_base": f"你是一个专业的多语言翻译专家，擅长将中文翻译成{target_lang}。请只返回翻译后的{target_lang}文本，不要包含任何解释或额外信息。",
            "system_with_examples": f"你是一个专业的多语言翻译专家，擅长将中文翻译成{target_lang}。{{examples_text}}请参考以上示例的翻译风格和质量，将用户输入的中文文本翻译成{target_lang}。请只返回翻译后的{target_lang}文本，不要包含任何解释或额外信息。",
            "example_label": "示例",
            "chinese_label": "中文",
    ...
    def create_system_prompt(self, source_lang, target_lang, current_text=None, dev_idx=None):
        """创建系统提示词，包含 few-shot 示例，支持多语言提示词"""
        
        # 根据配置决定是否使用多语言提示词
        if self.use_multilingual_prompts:
            # 获取语言特定的提示词模板
            lang_prompts = self.get_language_specific_prompts(target_lang)
        else:
            # 使用中文提示词模板
            lang_prompts = {
                "system_base": f"你是一个专业的多语言翻译专家，擅长将{source_lang}翻译成{target_lang}。请只返回翻译后的{target_lang}文本，不要包含任何解释或额外信息。",
                "system_with_examples": f"你是一个专业的多语言翻译专家，擅长将{source_lang}翻译成{target_lang}。{{examples_text}}请参考以上示例的翻译风格和质量，将用户输入的中文文本翻译成{target_lang}。请只返回翻译后的{target_lang}文本，不要包含任何解释或额外信息。",
                "example_label": "示例",
                "chinese_label": "中文",
                "target_label": target_lang
            }
        ...
        if self.n_shot == 0:
            # 使用基础提示词
            system_prompt = lang_prompts["system_base"]
        else:
            ...
            if examples:
                ...
                for i, (chinese, translated) in enumerate(examples, 1):
                    examples_text += f"{lang_prompts['example_label']}{i}:\n"
                    examples_text += f"{lang_prompts['chinese_label']}: {chinese}\n"
                    examples_text += f"{lang_prompts['target_label']}: {translated}\n\n"
                
                system_prompt = lang_prompts["system_with_examples"].format(examples_text=examples_text)
            else:
                # 如果没有找到示例，使用基础提示词
                system_prompt = lang_prompts["system_base"]
...
def translate_deepseek(n_shot=0, use_multilingual_prompts=False):
    """主翻译函数"""
    # 设置信号处理器
    ...
    # 创建DeepSeek翻译器
    translator = DeepSeekTranslator(API_KEY, BASE_URL, MODEL_NAME, n_shot=n_shot, use_multilingual_prompts=use_multilingual_prompts)
...
def sync_translate_one(text, source_lang, target_lang, n_shot=0, use_multilingual_prompts=False):
    """同步翻译单个文本（用于测试）"""
    translator = DeepSeekTranslator(API_KEY, BASE_URL, MODEL_NAME, n_shot=n_shot, use_multilingual_prompts=use_multilingual_prompts)
    return translator.translate_text(text, source_lang, target_lang)

def test_translation(n_shot=0, use_multilingual_prompts=False):
    """测试翻译功能"""
    print("测试DeepSeek翻译...")
    if n_shot > 0:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--use-multilingual-prompts", action="store_true",default=True,
                       help="使用多语言提示词模式（针对不同目标语言使用相应语言的提示词）")
    ...
    if args.precompute_bleu2_fast:
        ...
        test_translation(n_shot=n_shot, use_multilingual_prompts=args.use_multilingual_prompts)
    else:
        translate_deepseek(n_shot=n_shot, use_multilingual_prompts=args.use_multilingual_prompts)