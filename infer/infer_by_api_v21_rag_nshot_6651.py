...
def get_output_path(n_shot):
    if dev_mode:
        return os.path.join(base_dir, f"output/deepseek/dev_p21_t0_rag_{n_shot}shot.csv")
    else:
...
class DeepSeekTranslator:
    ...
    def create_system_prompt(self, source_lang, target_lang, current_text=None, dev_idx=None):
        """创建优化的系统提示词，包含 few-shot 示例"""
        
        # 根据目标语言定制的特殊指令
        lang_specific_instructions = {
            "英语": "请确保翻译自然流畅，符合英语表达习惯。",
            "English": "请确保翻译自然流畅，符合英语表达习惯。",
            "马来语": "请使用标准马来语(Bahasa Malaysia)，注意语法和词汇的准确性。",
            "Malay": "请使用标准马来语(Bahasa Malaysia)，注意语法和词汇的准确性。",
            "泰语": "请使用标准泰语，注意泰语的语序和敬语使用。",
            "Thai": "请使用标准泰语，注意泰语的语序和敬语使用。"
        }
        
        lang_instruction = lang_specific_instructions.get(target_lang, "请确保翻译准确、自然。")
        
        if self.n_shot == 0:
            # 优化后的0-shot系统提示词
            system_prompt = f"""你是一位专业的多语言翻译专家，精通中文到{target_lang}的准确翻译。

翻译要求：
1. 准确性：确保翻译内容准确，不遗漏、不增加原文信息
2. 自然性：译文应符合{target_lang}的表达习惯和语法规则
3. 一致性：保持术语和表达方式的一致性
4. {lang_instruction}

输出格式：只输出翻译结果，不包含任何解释、注释或额外信息。"""
        else:
...
3. 一致性：保持术语和表达方式的一致性
4. {lang_instruction}
5. 风格参考：请参考以下示例的翻译风格和质量标准
{examples_text}

输出格式：只输出翻译结果，不包含任何解释、注释或额外信息。"""
            else:
                # 如果没有找到示例，使用优化的0-shot提示词
                system_prompt = f"""你是一位专业的多语言翻译专家，精通中文到{target_lang}的准确翻译。
