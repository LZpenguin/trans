...
def get_output_path(n_shot):
    return os.path.join(base_dir, f"output/deepseek/dev_p2_t0_{n_shot}shot.csv")
...
class DeepSeekTranslator:
    ...
    def load_examples(self, target_lang):
        """从 dev.csv 加载指定语言的翻译示例"""
        if target_lang in self.examples_cache:
            return self.examples_cache[target_lang]
            
        examples = []
        if self.n_shot > 0 and os.path.exists(shot_path):
            try:
                shot_df = pd.read_csv(shot_path)
                # 筛选出目标语言的数据，且有有效翻译结果的
                lang_data = shot_df[
                    (shot_df['语言'] == target_lang) &  # 语言列匹配
                    (shot_df['文本'].notna()) &  # 有翻译结果
                    (shot_df['文本'] != "") &  # 翻译结果非空
                    (shot_df['中文'].notna()) &  # 中文文本非空
                    (shot_df['中文'] != "")
                ]
        ...
        self.examples_cache[target_lang] = examples
        return examples
    ...
    def create_system_prompt(self, source_lang, target_lang):
        ...
        if self.n_shot == 0:
            ...
            # 包含示例的系统提示词
            examples = self.load_examples(target_lang)
...
def test_translation(n_shot=0):
    ...
    for i, (text, source_lang, target_lang) in enumerate(test_cases, 1):
        ...
        # 获取完整的系统提示词
        system_prompt = translator.create_system_prompt(source_lang, target_lang)
